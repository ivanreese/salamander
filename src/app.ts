...
const elm = document.querySelector("canvas") as HTMLCanvasElement
...
function bind(label: string, binding: number, values: ArrayBufferView, type: GPUBufferBindingType = "uniform", visibility = visC | visF): Bound {
  ...
  return { buffer, binding, values }
}

function rebind(bound: Bound, vals: ArrayBufferView = bound.values) {
  bound.values = vals
